...
struct SpiderFetchTask
{
    ...
    struct addrinfo* ai_;
    // 抓取请求列表
    std::vector<SpiderFetchRequest> req_array_;
    // 解析器类型
    ...
    union
    {
        SpiderTaskOption option_;
        uint64_t   op_val_;   
    };

    SpiderFetchTask(): ver_(REQUEST_VERSION), ai_(NULL), parser_version_(0), op_val_(0) 
    {
    ...
    void add_request(const SpiderFetchRequest& req)
    {
